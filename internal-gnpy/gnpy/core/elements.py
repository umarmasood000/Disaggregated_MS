...
class Fiber(_Node):
    def __init__(self, *args, params=None, **kwargs):
        ...
        if self.params.raman_efficiency:
            ...
            self._cr_function = interp1d(frequency_offset, cr, bounds_error=False, fill_value=0.0)
        else:
            self._cr_function = self.cr_function
    ...
    @staticmethod
    def cr_function(frequency):
        return zeros(squeeze(frequency).shape)
    ...
    def loss(self, frequency=None):
        """total loss including padding att_in: useful for polymorphism with roadm loss"""
        frequency = self.params.ref_frequency if frequency is None else asarray(frequency)
        
        return self.loss_coef_func(frequency) * self.params.length + \
    ...
    def beta2(self, frequency=None):
        frequency = self.params.f_dispersion_ref if frequency is None else asarray(frequency)
        if self.params.dispersion.size > 1:
            try:
                beta2 = interp1d(self.params.f_dispersion_ref, self.params.beta2)(frequency)
            except ValueError:
                raise SpectrumError('The spectrum bandwidth exceeds the frequency interval used to define the fiber '
                                    f'dispersion in "{type(self).__name__} {self.uid}".'
                                    f'\nSpectrum f_min-f_max: {round(frequency[0]*1e-12,2)}-'
                                    f'{round(frequency[-1]*1e-12,2)}'
                                    f'\nDispersion f_min-f_max: {round(self.params.f_dispersion_ref[0]*1e-12,2)}-'
                                    f'{round(self.params.f_dispersion_ref[-1]*1e-12,2)}')
        else:
            beta2 = full(frequency.size, self.params.beta2)
        return squeeze(beta2)

    def beta3(self, frequency=None):
        frequency = self.params.f_dispersion_ref if frequency is None else asarray(frequency)
        if self.params.dispersion.size > 1:
            try:
                beta3 = interp1d(self.params.f_dispersion_ref, self.params.beta3)(frequency)
            except ValueError:
                raise SpectrumError('The spectrum bandwidth exceeds the frequency interval used to define the fiber '
                                    f'dispersion in "{type(self).__name__} {self.uid}".'
                                    f'\nSpectrum f_min-f_max: {round(frequency[0]*1e-12,2)}-'
            ...
            beta3 = full(frequency.size, self.params.beta3)
        return squeeze(beta3)

    def f_beta_ref(self, frequency=None):
        frequency = self.params.f_dispersion_ref if frequency is None else asarray(frequency)
        if self.params.f_dispersion_ref.size > 1:
            f_beta_ref = frequency
        else:
            f_beta_ref = full(frequency.size, self.params.f_dispersion_ref)

        return squeeze(f_beta_ref)

    def gamma(self, frequency=None):
        frequency = self.params.f_dispersion_ref if frequency is None else asarray(frequency)
        if self.params.dispersion.size > 1:
            try:
                gamma = interp1d(self.params.f_gamma_ref, self.params.gamma)(frequency)
            except ValueError:
                raise SpectrumError('The spectrum bandwidth exceeds the frequency interval used to define the fiber '
                                    f'gamma in "{type(self).__name__} {self.uid}".'
                                    f'\nSpectrum f_min-f_max: {round(frequency[0] * 1e-12, 2)}-'
                                    f'{round(frequency[-1] * 1e-12, 2)}'
                                    f'\nGamma f_min-f_max: {round(self.params.f_gamma_ref[0] * 1e-12, 2)}-'
                                    f'{round(self.params.f_dispersion_ref[-1] * 1e-12, 2)}')
        else:
            gamma = full(frequency.size, self.params.gamma)
        return squeeze(gamma)
    ...
    def chromatic_dispersion(self, frequency=None):
        ...
        if self.params.dispersion.size > 1:
            try:
                dispersion = interp1d(self.params.f_dispersion_ref, self.params.dispersion)(frequency)
            except ValueError:
                raise SpectrumError('The spectrum bandwidth exceeds the frequency interval used to define the fiber '
                                    f'dispersion in "{type(self).__name__} {self.uid}".'
                                    f'\nSpectrum f_min-f_max: {round(frequency[0]*1e-12,2)}-'
            ...
            dispersion = full(frequency.size, self.params.dispersion - c / self.params.f_dispersion_ref ** 2 *
                              self.params.dispersion_slope * (frequency - self.params.f_dispersion_ref))
        return dispersion * self.params.length
