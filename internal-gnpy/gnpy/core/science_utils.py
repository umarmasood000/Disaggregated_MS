...
from numpy import interp, pi, zeros, shape, where, cos, array, append, ones, exp, arange, sqrt, trapz, arcsinh, \
    clip, abs, sum, concatenate, flip, outer, inner, transpose, max, format_float_scientific, diag, unique, argsort, \
    cumprod, log, reshape, swapaxes, full
...
DEFAULT_SMP_WEIGHT = (16.0 / 27.0)
DEFAULT_XMP_WEIGHT = 2 * (16.0 / 27.0)
...
class NliSolver:
    """ This class implements the NLI models.
        ...
        'ggn_approx: eq. 24/25 from https://ieeexplore.ieee.org/document/9741324
    """
    ...
    def compute_nli(spectral_info: SpectralInformation, srs: StimulatedRamanScattering, fiber):
        ...
        alpha = fiber.alpha(spectral_info.frequency)
        beta2 = fiber.beta2(spectral_info.frequency)
        beta3 = fiber.beta3(spectral_info.frequency)
        f_ref_beta = fiber.f_beta_ref(spectral_info.frequency)
        gamma = fiber.gamma(spectral_info.frequency)
        length = fiber.params.length

        if 'gn_model_analytic' == sim_params.nli_params.method:
            nli = NliSolver._gn_analytic(spectral_info=spectral_info, alpha=alpha, beta2=beta2, gamma=gamma,
                                         length=length)
        elif 'ggn_spectrally_separated' in sim_params.nli_params.method:
            nli = NliSolver._ggn_spectrally_separated(spectral_info=spectral_info, srs=srs, alpha=alpha, beta2=beta2,
                                                      beta3=beta3, f_ref_beta=f_ref_beta, gamma=gamma)
        elif 'ggn_approx' in sim_params.nli_params.method:
            nli = NliSolver._ggn_approx(spectral_info=spectral_info, srs=srs, alpha=alpha, beta2=beta2, gamma=gamma,
                                        f_ref_beta=f_ref_beta)
        else:
    ...
    def _psi(df, baud_rate, beta2, effective_length, asymptotic_length):
        ...
        pump_beta2 = outer(ones(beta2.size), beta2)
        pump_baud_rate = baud_rate
        right_extreme = df + pump_baud_rate / 2
        left_extreme = df - pump_baud_rate / 2
        psi = (arcsinh(pi ** 2 * asymptotic_length * abs(pump_beta2) * cut_baud_rate * right_extreme) -
               arcsinh(pi ** 2 * asymptotic_length * abs(pump_beta2) * cut_baud_rate * left_extreme)) / 2
        psi *= effective_length ** 2 / (2 * pi * abs(pump_beta2) * asymptotic_length)
        return psi

    # Methods for computing the GGN-model
    @staticmethod
    def _ggn_spectrally_separated(spectral_info: SpectralInformation, srs: StimulatedRamanScattering, alpha, beta2,
                                  beta3, f_ref_beta, gamma):
        """ Computes the nonlinear interference power evaluated at the fiber input.
        ...
        g_nli = array([])
        f_nli = array([])
        for i, cut_carrier in enumerate(cuts):
            index = cut_carrier.channel_index
            beta2_cut = beta2[index]
            beta3_cut = beta3[index]
            gamma_cut = gamma[index]
            f_ref = f_ref_beta[index]
            ...
            for j, pump_carrier in enumerate(spectral_info.carriers):
                ...
                f_cut_resolution = min(k_tol, phi_tol) / abs(beta2_cut) / (4 * pi ** 2 * (1 + dn) * slot_width)
                f_pump_resolution = min(k_tol, phi_tol) / abs(beta2_cut) / (4 * pi ** 2 * slot_width)
                if dn == 0:  # SPM
                    ggg = g_cut ** 3 * gamma_cut ** 2
                    g_nli_computed += \
                        spm_weight * ggg * NliSolver._generalized_psi(f_eval, cut_carrier, pump_carrier,
                                                                      f_cut_resolution, f_pump_resolution,
                                                                      srs, alpha[j], beta2_cut, beta3_cut, f_ref)
                else:  # XPM
                    g_pump = (pump_carrier.power.signal / pump_carrier.baud_rate)
                    ggg = g_cut * g_pump ** 2 * gamma_cut ** 2
                    frequency_offset_threshold = NliSolver._frequency_offset_threshold(beta2_cut,
                                                                                       pump_carrier.baud_rate)
                    if abs(delta_f) <= frequency_offset_threshold:
                        g_nli_computed += \
                            xpm_weight * ggg * NliSolver._generalized_psi(f_eval, cut_carrier, pump_carrier,
                                                                          f_cut_resolution, f_pump_resolution,
                                                                          srs, alpha[j], beta2_cut, beta3_cut,
                                                                          f_ref)
                    else:
                        g_nli_computed += \
                            xpm_weight * ggg * NliSolver._fast_generalized_psi(f_eval, cut_carrier, pump_carrier,
                                                                               f_cut_resolution, srs, alpha[j],
                                                                               beta2_cut, beta3_cut, f_ref)
            f_nli = append(f_nli, cut_carrier.frequency)
            g_nli = append(g_nli, g_nli_computed)
        if g_nli.size > 1:
            g_nli = interp1d(f_nli, g_nli, kind='linear', fill_value='extrapolate')(spectral_info.frequency)
        else:
            g_nli = full(spectral_info.frequency.size, g_nli[0])
    ...
    @staticmethod
    def _ggn_approx(spectral_info: SpectralInformation, srs: StimulatedRamanScattering, alpha, beta2, gamma,
                    f_ref_beta):
        """ Computes the nonlinear interference power evaluated at the fiber input.
        The method uses eq. 24 from https://ieeexplore.ieee.org/document/9741324"""
        cut_indices = array(sim_params.nli_params.computed_channels) - 1 if sim_params.nli_params.computed_channels \
            else array(spectral_info._channel_number) - 1
        cuts = [carrier for carrier in spectral_info.carriers if carrier.channel_number
                in sim_params.nli_params.computed_channels] if sim_params.nli_params.computed_channels \
            else spectral_info.carriers
        slot_width = max(spectral_info.slot_width)
        ...
        psi = NliSolver._approx_psi(df=df, frequency=spectral_info.frequency, beta2=beta2, baud_rate=baud_rate,
                                    loss_profile=srs.loss_profile, z=srs.z)

        for cut_index, cut in zip(cut_indices, cuts):
            k_tol = dispersion_tolerance * abs(alpha[cut_index])
            phi_tol = phase_shift_tolerance / delta_z
            f_cut_resolution = min(k_tol, phi_tol) / abs(beta2[cut_index]) / (4 * pi ** 2 * slot_width)
            f_pump_resolution = min(k_tol, phi_tol) / abs(beta2[cut_index]) / (4 * pi ** 2 * slot_width)
            psi[cut_index, cut_index] = NliSolver._generalized_psi(cut.frequency, cut, cut, f_cut_resolution,
                                                                   f_pump_resolution, srs, alpha[cut_index],
                                                                   beta2[cut_index], 0, f_ref_beta[cut_index])
        g_nli = weight * gamma ** 2 * ggg * psi
        g_nli = g_nli[cut_indices, :]
        g_nli = sum(g_nli, 1)
        # g_nli = interp1d(spectral_info.frequency[cut_indices], g_nli, kind='linear',
        #                  fill_value='extrapolate')(spectral_info.frequency)
        if g_nli.size > 1:
            g_nli = interp1d(spectral_info.frequency[cut_indices], g_nli, kind='linear',
                             fill_value='extrapolate')(spectral_info.frequency)
        else:
            g_nli = full(spectral_info.frequency.size, g_nli[0])
        nli = baud_rate * g_nli  # Local white noise
    ...
    def _approx_psi(df, frequency, baud_rate, beta2, loss_profile, z):
        ...
        The method uses eq. 25 from https://ieeexplore.ieee.org/document/9741324"""
