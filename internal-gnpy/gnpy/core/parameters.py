...
from numpy import asarray, array, interp, full
...
class FiberParams(Parameters):
    def __init__(self, **kwargs):
        try:
            ...
            self._dispersion_slope = asarray(kwargs.get('dispersion_slope', 0))  # s/m/m/m
            if self._dispersion_slope.size != self._f_dispersion_ref.size:
                raise ParametersError('When the dispersion is defined along the frequency, the dispersion slope '
                                      'must be defined for each dispersion reference frequency or just omitted.')

            self._beta2 = -((c / self._f_dispersion_ref) ** 2 * self._dispersion) / (2 * pi * c)  # 1/(m * Hz^2)
            ...
            self._beta3 = (self._dispersion_slope - (4*pi * self._f_dispersion_ref**3 / c**2) * self._beta2) / \
                          (2*pi * self._f_dispersion_ref**2 / c)**2
            ...
            if self._effective_area:
                ...
                if type(kwargs['gamma']) == dict:
                    self._gamma = kwargs['gamma']['value']  # 1/W/m
                    self._f_gamma_ref = kwargs['gamma']['frequency']  # Hz
                    ref_gamma = interp(self._ref_frequency, self._f_gamma_ref, self._gamma)
                    self._effective_area = 2 * pi * n2 / (self.ref_wavelength * ref_gamma)  # m^2
                else:
                    self._gamma = asarray(kwargs['gamma'])  # 1/W/m
                    self._f_gamma_ref = asarray(self._ref_frequency)  # Hz
    ...
    @property
    def f_gamma_ref(self):
        return self._f_gamma_ref
